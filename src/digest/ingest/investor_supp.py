...
def _detect_metric(table: Table) -> str:
    """'paid' | 'incurred' from a triangle's header. Defaults to 'incurred'
    (the more commonly disclosed development basis) when ambiguous."""
    joined = " ".join(table.header).lower()
...
def _detect_lob(table: Table) -> str:
    """Line of business from a triangle's header text; 'all_lines' fallback."""
    joined = " ".join(table.header).lower()
