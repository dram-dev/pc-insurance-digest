...
def _fetch_series(entry: dict) -> list[tuple[str, float]]:
    """Fetch an index's observation series. SCAFFOLD: returns [] until the
    per-source scraper is implemented + validated on the Mac mini. Wire each
    source's parser here keyed on entry['source']."""
